function [data_output,frames,data_merged, PixelCallibration] = HSMdrift(data)
    ...
    for i = 1:frames
        if i == 1   % because the first frame is already uploaded in I
            bg = mean(mean(I));
            data_output(:,:,i) = I;
            img_h2 = round(data_output(round(size(I,1)*0.25):round(size(I,1)*0.75),round(size(I,2)*0.25):round(size(I,2)*0.75),i)-bg);
            %img_h2(img_h2<100) = 0;
            %img_h2 = img_h2*10;
            %img_h2 = BackgroundCorrection_WaveletSet(data_output(round(size(I,1)*0.25):round(size(I,1)*0.75),round(size(I,2)*0.25):round(size(I,2)*0.75),i),4);
            img_h(:,:,i) = img_h2;
        else
            data_output(:,:,i) = bfGetPlane(data, i);  % uploading of other frames
            bg = mean(mean(data_output(round(size(I,1)*0.25):round(size(I,1)*0.75),round(size(I,2)*0.25):round(size(I,2)*0.75),i)));
            img_h2 = round(data_output(round(size(I,1)*0.25):round(size(I,1)*0.75),round(size(I,2)*0.25):round(size(I,2)*0.75),i) - bg);
            %img_h2(img_h2<100) = 0;
            %img_h2 = img_h2*10;
            %img_h2 = BackgroundCorrection_WaveletSet(data_output(round(size(I,1)*0.25):round(size(I,1)*0.75),round(size(I,2)*0.25):round(size(I,2)*0.75),i),4);
            ...
            [img_before, ~] = BackgroundCorrection_WaveletSet(img_h(:,:,i-1), 4);
            [img_after,~] = BackgroundCorrection_WaveletSet(img_h(:,:,i),4);
            % cross correlation between following frames
            %C = xcorr2(double(img_h(:,:,i-1)),double(img_h(:,:,i)));
            C = normxcorr2(img_after,img_before);
            %C =  normxcorr2(double(img_h(:,:,i-1)),double(img_h(:,:,i)));
            %C = BackgroundCorrection_WaveletSet(img_h,4);
